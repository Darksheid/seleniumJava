...
public class googlePage {
	...
	@FindBy(xpath = "//img[@class='python-logo']")
	private WebElement python_logo;
	...
	public boolean navigateGooglePage(String url) throws InterruptedException {
		boolean res = false;
		try {
			System.out.println("Launching Python Homepage----------------");
			driver.get(url);
			wait.until(ExpectedConditions.visibilityOf(python_logo));
			System.out.println("Python Homepage is launched sucessfully!!");
			res = true;
		} catch (Exception e) {
			e.printStackTrace();
